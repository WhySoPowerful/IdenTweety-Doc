...
# Set input section
with input_section:
    st.markdown("<h2 style='color: #33C5FF;'>Enter a Twitter Handle</h2>", unsafe_allow_html=True)
    ...
    if st.button('Identify'):
        if user_input:
            ...
            try:
                ...
                st.markdown(f"<h3 style='color: var(--main-text-color);'>@{user_input}</h3>", unsafe_allow_html=True)
                st.markdown(f"<p style='color: var(--secondary-text-color);'>{user.name}</p>", unsafe_allow_html=True)
                st.markdown(f"<p style='color: var(--secondary-text-color);'>{user.description}</p>", unsafe_allow_html=True)
                st.markdown("<hr style='border: 1px solid #33C5FF; border-radius: 2px;'>", unsafe_allow_html=True)
                st.write('')
